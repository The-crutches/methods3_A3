...
data <- read.delim("Pitch/Study1D0S101T1_f0.txt")




test <- as.data.frame(mean(first$time))
test$time_sd <- sd(first$time)
test$time_min <- min(first$time)
test$time_max <- max(first$time)
test$time_iqr <- IQR(first$time)
test$time_median <- median(first$time)
test$time_meanAD <- mad(first$time)
test$time_cv <- sd(first$time, na.rm = TRUE / mean(first$time, na.rm = TRUE))*100


test$f0_mean <- mean(first$f0)
test$f0_sd <- sd(first$f0)
test$f0_min <- min(first$time)
test$f0_max <- max(first$time)
test$f0_iqr <- IQR(first$f0)
test$f0_median <- median(first$f0)
test$f0_meanAD <- mad(first$f0)
test$f0_cv <- sd(first$f0, na.rm = TRUE / mean(first$time, na.rm = TRUE))*100
...
demo <-  read.csv("~/Uni stuff/Expmeth3/methods3_A3/DemographicData.csv", sep = ";")

demo <- demo %>%  
    rename(Subject = Participant) %>% 
    filter(is.na(Diagnosis) == F) %>% 
    mutate(
        Subject = as.numeric(Subject),
        Study = as.numeric(Study),
        Diagnosis = as.factor(Diagnosis)
...
art <- art %>% mutate(
    Study = substr(art$soundname, 6,6),
    Study = as.numeric(Study),
    Diagnosis = substr(art$soundname, 8,8),
    Diagnosis = recode(Diagnosis, 
               '0' = 'Control',
               '1' = 'Schizophrenia'),
    Diagnosis = as.factor(Diagnosis),
    Subject = substr(art$soundname, 10,12),
    Subject = as.numeric(Subject),
...
pitch_data <- read.csv("pitch_data_clean.csv")

# Now we merge them
# creating new IDs
pitch_data$new_ID <- paste0(pitch_data$Diagnosis, pitch_data$Subject)
demo$new_ID <- paste0(demo$Diagnosis, demo$Subject)
art$new_ID <- paste0(art$Diagnosis, art$Subject)
...
df1 <- merge(df, art, by = c('new_ID','Study', 'Diagnosis', 'Trial'), all = T)
...
```{r}
# there are no NA's in subject, diagnosis and study for DEMO
...
df_no_na_1 %>% split(df_no_na_1$Diagnosis) %>% map(summary)
