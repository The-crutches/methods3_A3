...
```{r}
#We found that study 3 and 4 have no data for several of the demographic observations, as we don't use any of those columns with NA's, we decided to keep the observations 

#Insert Nannas and Kristines data 
```
...
proportion_spoken <- lmerTest::lmer(prop_spoken_time ~ 0 + Diagnosis + (1|ID), df, REML = FALSE)
    ...
    #As it is unclear whether we had to have a proportion of spoken time or just spoken time as phonation time, we make two models 

spoken <- lmerTest::lmer(df$phonationtime..s.~0 + Diagnosis + (1|ID), df, REML = FALSE )
