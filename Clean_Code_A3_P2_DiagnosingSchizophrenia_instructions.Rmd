...
```
```{r}
#Making dataframe with only the predictors we are interested 
dff <- df %>% select(Diagnosis1, ID, speechrate, pause_duration, prop_spoken_time, pitch_IQR)

#Partitioning 
p1 <- partition(dff, p = 0.2, cat_col = 'Diagnosis1', id_col = 'ID')
  #Making test set 
df_test <- p1[[1]]
  #Making train set 
df_train <- p1[[2]]
...
f0 <- glmer(Diagnosis1 ~ speechrate + pause_duration + pitch_IQR + (1|ID), family = 'binomial', dff)
f1 <- glmer(dffgnosis1 ~ speechrate + pause_duration + (1|ID), family = 'binomial', dff)
f2 <- glmer(Diagnosis1 ~ speechrate + pitch_IQR + (1|ID), family = 'binomial', dff)
f3 <- glmer(Diagnosis1 ~ pause_duration + pitch_IQR + (1|ID), family = 'binomial', dff)
...
cv_4 <- cross_validate_fn(
  ...
  formulas = c("Diagnosis1 ~ speechrate + pitch_IQR",
               "Diagnosis1 ~ pitch_IQR + pause_duration"),
  type = "binomial",
