...
# load data
    data <- read.delim("Pitch/Study1D0S101T1_f0.txt", header = T)
    filename <- filename
...
```{r}
pacman::p_load(tidyverse, fs, readr, EnvStats, DescTools, purrr)
...
# when you've created a function that works, you can
?list.files
pitch_data = list.files(path = "Pitch/", pattern = ".txt", all.files = T) %>% purrr:: map_df(read_pitch)
...
#Merging
df1 <- merge(art, demo, all = T)
df <- merge(df1, pitch, all = T)
    #Getting 4492 observation of 38 variables 
    #We have 3.826 files 
...
#Counting participants, how many schizo and how many controls?
df %>% 
  split(df$Diagnosis) %>% 
  map(summary) 
#Gender, age, symptom severity 

#Group by diagnosis and study 
...
- speech rate (lower, Hedges' g: -0.75, 95% CIs: -1.51, 0.04)
...
#Plan man 
    #Scaling 
    #Random intercept as study and as subject 
        #Not analyzing both languages, only Danish. Many language developments change with the language, doesn't make any sense to do it over both. 
    #Does it make sense to scale all of the varibles? 

#Questions: 
    #Do we need to calculate the hedges G thingy?? We don't know what that is... But you cant compare apples and bananas 
    #
```
...
library(lme4)
#Models to run 
    #Syntax to use if we want to study the studies seperately 
