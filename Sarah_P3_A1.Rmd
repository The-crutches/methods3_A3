...
# testing regex
files <- list.files('Pitch/')
df <- cbind(files, pitch_data)

df <- df %>% mutate(
  ID = substr(df$files, 10, 12), #this doesn't hold up, more than 1 Subj number, plus the extra files are messed up
  study = substr(df$files, 6, 6),
  diagnosis = substr(df$files, 8, 8),
  trial = substr(df$files, '^S', '[_]') #this bot doesn't work either, need regex of some sort
                    )
...
#demo <- demo %>% fill(SANS)  can fill in data, but we don't want that apparently
...
# Now we merge them
demo$ID <- paste0(demo$Participant, demo$Diagnosis)
art$ID <- paste0(art$Subject,art$Diagnosis)
pitch$ID <- paste0(pitch$Subject, pitch$Diagnosis)

one <- merge(demo, pitch, all = F)
df <- merge(one, art, all = F)
# Now we save them
...
```{r}
df$Diagnosis <- na.omit(df$Diagnosis)
df$ID <- na.omit(df$ID)
df$Gender <- na.omit(df$Gender)
