...
# participant 342 has numerous trials registered? and an odd filename, resulting in error

# Finally the pitch data
pitch <- read.csv('pitch_data_clean.csv')

pitch <- pitch %>% 
  filter(Study <= 4, Subject != 342)

#something wrong with 342 also
#removing him, because the file is unreliable, can't tell which trial is correct
```
...
# left join: make sure the most important dataset comes first, as it drops values in right dataset, if there isn't a match in left dataset- 
...
ggplot(df, aes(x = Diagnosis, y = pause_duration, colour = Diagnosis)) +
  geom_boxplot(width = 0.5) +
  ggtitle("xxxx")+
  stat_summary(fun.y = mean, geom = "point", shape = 23, colour = "Black")

ggplot(df, aes(x = Diagnosis, y = prop_spoken_time, colour = Diagnosis)) +
  geom_boxplot(width = 0.5) +
  ggtitle("xxxx")+
  stat_summary(fun.y = mean, geom = "point", shape = 23, colour = "Black")

ggplot(df, aes(x = Diagnosis, y = speechrate..nsyll.dur., colour = Diagnosis)) +
  geom_boxplot(width = 0.5) +
  ggtitle("xxxx")+
  stat_summary(fun.y = mean, geom = "point", shape = 23, colour = "Black")

ggplot(df, aes(x = Diagnosis, y = pitch_IQR, colour = Diagnosis)) +
  geom_jitter()
```
...
```{r}
desc <- df %>% split(df$Diagnosis) %>% map(summary) 
desc
...
```{r}
p_load(tidymodels, lme4, tidyverse, EMAtools)

df <- df %>% mutate_if(is.numeric, scale)

m1 <- lme4::lmer(pitch_IQR ~ 0 + Diagnosis:Study + (1|ID), data = df)
summary(m1)

k <- lme.dscore(m1, data = df, type = 'lme4')
l
#pitch variability - all pitch values? pitchIQR, cv
...
# should we select out variables like ID and Subject?
#also the trial and f0 column that randomly appeared somewhere?

m <- as.tibble(df)
m <- df %>% select(-time, -f0, -Language, -Study, -Trial, -Subject)

m_split <- initial_split(m)

m_split %>% training() %>% glimpse()

test <- m_split %>% testing() %>% glimpse()
...
testing_r <- m_recipe %>% bake(testing(m_split))
training_r <- bake(m_recipe, new_data = m)


juiced_training <- juice(training_r) 
...
pitch_variability <- lmerTest::lmer(pitch_IQR ~ 0 + Diagnosis + (1|ID), df)
...
proportion_spoken <- lmerTest::lmer(prop_spoken_time ~ 0 + Diagnosis + (1|ID), df)
summary(proportion_spoken)

#speech rate

speech_rate <- lmerTest::lmer(speechrate..nsyll.dur. ~ 0 + Diagnosis + (1|ID), df)
summary(speech_rate)

#pause duration
    #makin new column with pause duration -> (Duration - Spoken Duration) / PauseN
df$pause_duration <- (df$dur..s. - df$phonationtime..s.) / df$npause

pause_duration <- lmerTest::lmer(pause_duration ~ 0 + Diagnosis + (1 | ID), df)
