...
first <- read.delim('Pitch/Study1D0S101T1_f0.txt')
ex2 <- read.delim('Pitch/Study1D1s111T1_f0.txt', header = T)

ex <- convert("Pitch/Study1D0S101T1_f0.txt", "Pitch/Study1D0S101T1_f0.csv")

x <-list.files('Test') [1]
x <- as.data.frame(x)

data_path <- "Pitch/"   # path to the data
files <- dir(data_path) # get file names

data <- data %>% mutate(file = files)
data$filename <- bind_cols(files)

data <- files %>%
  # read in all the files, appending the path before the filename
  map(~ read.delim(file.path(data_path, .))) %>% 
  reduce(rbind)

  
data <- data_frame(filename = 'Test') %>% # create a data frame
...
x <- list.files('Pitch/', full.names = T)
?list.files
```
...
pitch_data = list.files(path = "Pitch/",pattern = ".txt", 
                        full.names = T) %>% ## NB replace with your path to the files
