...
#CLEAN THIS UGLY PUDDING 

library(pastecs)

report<- group_by(df, Diagnosis)

control_Age <- na.omit(control)

summarize(Schizo, sd(Age), mean(Age))

summarize(report, sd(SANS), mean(SANS))
summarize(report, sd(SAPS), mean(SAPS))
...
length(unique(report$Gender))

control <- subset(report, Diagnosis == "Control")
length(unique(schizo_fe$ID))



summarize(control, sd(VerbalIQ), mean(VerbalIQ))

summarize(report, sd(NonVerbalIQ), mean(NonVerbalIQ))
Schizo$Age <- as.numeric(Schizo$Age)
...
```
```{r}
#2 - accoustic profile of schizophrenic voice 

#make a range column 

df$Range <- df$pitch_max- df$pitch_min 

#make a bar plot showing  for mean pitch Control and Schizophrenia 

ggplot(df, aes(x = Diagnosis, y = pitch_mean, fill = Diagnosis)) +
 geom_bar(stat = "summary", fun.y = mean)+
  geom_errorbar(stat = "summary", fun.data = mean_se)

df <-group_by(df, Diagnosis)

summarise(df, mean(pitch_mean), sd(pitch_mean))

df$rangesd <- sd(df$Range)

summarize(df, mean(rangesd))
...
t.test(df$pitch_IQR ~ df$Diagnosis)
...
t.test(df$prop_spoken_time ~ df$Diagnosis)
...
t.test(df$phonationtime..s. ~ df$Diagnosis)
...
t.test(df$speechrate..nsyll.dur. ~ df$Diagnosis)
...
t.test(df$pause_duration ~ df$Diagnosis)
