...
```{r}
mydir = "Pitch" #(navnet på mappen i directory)
myfiles = list.files(path=mydir, pattern="*.txt", full.names=TRUE)

Pitch = ldply(myfiles, read.delim)

Pitch["study"] <- study



study <- data.frame(substr(myfiles, 12,12))
diagnosis <- data.frame(substr(myfiles, 14,14))
ID <- data.frame(substr(myfiles, 15,18))
trial <- data.frame(substr(myfiles, 20,21))
trial <- str_replace_all(trial$substr.myfiles..19..21., "[_]" , " ")

filenames <- select = c(study, diagnosis, ID, trial)

colnames(filenames)[colnames(filenames)=="substr.myfiles..12..12."] <- "Study"
colnames(filenames)[colnames(filenames)=="substr.myfiles..14..14."] <- "Diagnosis"
colnames(filenames)[colnames(filenames)=="substr.myfiles..15..18."] <- "ID"


write.csv(filenames, "ParticipantInfo.csv")
